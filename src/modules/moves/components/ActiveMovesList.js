...
const ActiveMovesList = ({ moves, onMoveSelect, onUpdate, loading, error }) => {
  // Ensure moves is always an array
  const movesList = Array.isArray(moves) ? moves : [];

  return (
    <div className="space-y-6">
      {loading && (
        <div className="text-center py-12 bg-white rounded-lg">
          <p className="text-gray-500">Lade Umzüge...</p>
        </div>
      )}
      ...
      {!loading && !error && movesList.length === 0 && (
        <div className="text-center py-12 bg-white rounded-lg">
